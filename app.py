...
# Page configuration
st.set_page_config(
    page_title="Langmuir Scatter Explorer", page_icon="🔬", layout="wide"
)
...
st.markdown(
    """
**How to use**: Select variables for X-axis, Y-axis, color, and marker size.
Use **Log X/Y** to switch to logarithmic scales. Choose **All couples** or **Separate figures** for display mode.

Points colored by `log₁₀(Ω)` show Langmuir kinetics (Ω > 0). Black stars (★) = pure TASEP (Ω = 0).
By default, marker size reflects **segment position** along the chain (larger = closer to entry).
"""
)

with st.expander("Observable definitions"):
    st.markdown(
        """
**System parameters**: `segment` (position along chain, 1-30), `Ω` (Langmuir attachment/detachment rate)

**Global (chain-level)**: `Rg_chain` (radius of gyration), `D_eff` (COM diffusion coefficient), `ρ_mean` (mean TASEP occupation)

**Local (per 10-bead segment)**: `ρ_local` (TASEP occupation), `D_local` (segment diffusion), `Rg_local` (segment gyration), `n_neighbors` (spatial density within 3σ)

**MSD at fixed lag times**: `MSD(t=20)`, `MSD(t=100)`, `MSD(t=200)`, `MSD(t=1000)`, `MSD(t=2000)`, `MSD(t=6000)` — mean squared displacement of segment at lag time t

**Fluctuations**: `σ_ρ_act` (measured std of TASEP occupation), `σ_ρ_theory` (TASEP-Langmuir prediction), `σ_Rg_local` (std of segment gyration), `σ_ρ_spatial` (std of spatial density)
    """
    )
...
def load_data():
    ...
    df["couple"] = df.apply(
        lambda r: f"α_in={r['alpha_in']:.1f}, α_out={r['alpha_out']:.1f}", axis=1
    )
    return df
...
COLUMN_GROUPS = [
    ...
    (
        "Fluctuations",
        ["rho_local_std", "sigma_rho_theory", "Rg_local_std", "n_neighbors_3s_std"],
    ),
]
...
with col2:
    y_var = st.selectbox(
        ...
        options=selectable_cols,
        index=(
            selectable_cols.index("rho_local") if "rho_local" in selectable_cols else 1
        ),
        format_func=lambda x: labels.get(x, x),
    )
...
with col_opt4:
    display_mode = st.radio(
        "Display", options=["All couples", "Separate figures"], index=1, horizontal=True
    )
...
if len(plot_df) == 0:
    st.warning(
        f"No valid data for {labels.get(x_var, x_var)} vs {labels.get(y_var, y_var)}"
    )
else:
    # Build hover data
    ...
    if display_mode == "All couples":
        ...
        if len(df_tasep) > 0:
            for couple in df_tasep["couple"].unique():
                ...
                if size_var != "None" and size_var in couple_data.columns:
                    ...
                    size_min, size_max = (
                        plot_df[size_var].min(),
                        plot_df[size_var].max(),
                    )
                    if size_max > size_min:
                        tasep_size = 5 + 15 * (size_values - size_min) / (
                            size_max - size_min
                        )
                    else:
                        tasep_size = 10
        ...
        fig.update_layout(
            ...
            coloraxis_colorbar_title=(
                labels.get(color_var, color_var) if color_var != "None" else None
            ),
        ...
        for i, couple in enumerate(couples):
            ...
            if size_var != "None":
                ...
                if size_max > size_min:
                    normalized_size = 5 + 15 * (size_values - size_min) / (
                        size_max - size_min
                    )
                else:
                    normalized_size = 10
            ...
            marker_dict = dict(
                size=normalized_size, line=dict(width=0.5, color="white"), opacity=0.8
            )
            ...
            if color_var != "None":
                marker_dict.update(
                    ...
                    colorbar=dict(title=labels.get(color_var, color_var), len=0.9),
                )
        ...
        for i, couple in enumerate(couples):
            ...
            stat_cols[i].caption(
                f"**{couple}**: R={corr:.3f}, R²={corr**2:.3f}, N={len(couple_df)}"
            )
